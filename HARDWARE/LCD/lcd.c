...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode&0X80)gui_show_ptchar(x+(size/2)*t,y,lcddev.width,lcddev.height,0,color,size,'0',mode&0xf);//填充0
				else gui_show_ptchar(x+(size/2)*t,y,lcddev.width,lcddev.height,0,color,size,' ',mode&0xf);
 				continue;
			}else enshow=1; 
